...
const startGame = () => {
  ...
  // 隨機選擇一個單字
  const randomIndex = Math.floor(Math.random() * letterData.words.length);
  currentItem.value = {
    letter: letterData.letter,
    word: letterData.words[randomIndex],
    image: letterData.images[randomIndex],
    color: letterData.color
  };

  // Get current letter + 3 random others
  const others = alphabet.filter(a => a.letter !== letter);
  const shuffledOthers = others.sort(() => 0.5 - Math.random()).slice(0, 3);

  // Transform alphabet entries to flat objects
  ...
  const options = [currentItem.value, ...otherOptions];
  // Shuffle options
  ...
  setTimeout(() => {
    if (window.speechSynthesis) {
      ...
      const utterance = new SpeechSynthesisUtterance(`Which one is ${letter}?`);
...
const checkAnswer = (option) => {
  if (option.letter === currentItem.value.letter) {
...
<template>
  <div class="game-view">
    <button class="back-btn" @click="router.push(`/letter/${route.params.letter}`)">
      ← Back
    </button>

    <div v-if="currentItem" class="game-content">
      <h2 class="game-title">Which one is {{ currentItem.letter }}?</h2>
